...
namespace Plugin
{
    public class MyMqttClient
    {
        ...
        public async Task UploadTpTelemetryDataAsync(string deviceName, object obj)
        {
            ...
            _logger.LogInformation($"UploadTpTelemetryDataAsync Topic:gateway/telemetry,Payload:{JsonConvert.SerializeObject(telemetryData)}");
            await Client.EnqueueAsync(new MqttApplicationMessageBuilder().WithTopic($"gateway/telemetry")
                ...
                if (CanPubTelemetry(deviceName, device, sendModel))
                {
                    switch (_systemConfig.IoTPlatformType)
                    {
                        ...
                        case IoTPlatformType.ThingsPanel:
                            ...
                            {
                                if (payload.Values != null)
                                    await UploadTpTelemetryDataAsync(deviceName, payload.Values);
                            }
                        ...
                        case IoTPlatformType.OneNET:
                        default:
                            _logger.LogInformation($"{deviceName} PublishTelemetryAsync not support {_systemConfig.IoTPlatformType}");
                            break;
                    }
                }
