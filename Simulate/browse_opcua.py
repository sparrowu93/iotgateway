#!/usr/bin/env python3
"""
OPC UA Client to browse the local simulated OPC UA server
...
async def main():
    ...
    async with Client(url=server_url) as client:
        try:
            ...
            logger.info(f"Objects node ID: {objects.nodeid}")
            ...
            for node in children:
                try:
                    ...
                    node_id = node.nodeid
                    logger.info(f"Node: {node}, Name: {browse_name}, ID: {node_id}")
                    ...
                    if "MyDevice" in str(browse_name):
                        ...
                        for var in device_vars:
                            ...
                            var_id = var.nodeid
                            logger.info(f"Variable: {var_name}, Value: {var_value}, ID: {var_id}")
